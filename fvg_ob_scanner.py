...
DEFAULT_CONFIG: Dict[str, object] = {
    "exchange_name": "binance",  # binance | mexc
...
def make_exchange(exchange_name: str = "binance", rate_limit: bool = True, market_type: str = "swap"):
    if ccxt is None:
    ...
    if not hasattr(ccxt, name):
        raise ValueError(f"Unsupported exchange: {exchange_name}. Expected 'binance' or 'mexc'.")

    exchange_cls = getattr(ccxt, name)

    # For USDT-M perpetual futures, use swap/futures default type based on exchange support.
    default_type = "future" if name == "binance" else "swap"
    if market_type:
        default_type = str(market_type)
...
def get_usdt_perp_symbols(exchange) -> List[str]:
    ...
    symbols = [
        ...
        if meta.get("swap") and meta.get("quote") == "USDT" and meta.get("active", True) and (meta.get("linear", True))
    ]
    symbols.sort()
    return symbols
...
def _get_worker_exchange(exchange_name: str, rate_limit: bool, market_type: str):
    key = f"{exchange_name}:{market_type}:{int(rate_limit)}"
    cache = getattr(_THREAD_LOCAL, "exchange_cache", None)
    if cache is None:
        cache = {}
        _THREAD_LOCAL.exchange_cache = cache
    ex = cache.get(key)
    if ex is None:
        ex = make_exchange(exchange_name=exchange_name, rate_limit=rate_limit, market_type=market_type)
        cache[key] = ex
    return ex


def _worker_scan_symbol(symbol: str, config: Dict[str, object], fetch_limit: int) -> Tuple[List[Signal], List[ZoneEvent]]:
    ex = _get_worker_exchange(str(config.get("exchange_name", "binance")), bool(config["rate_limit"]), str(config.get("market_type", "swap")))
...
def run_scan(config: Dict[str, object], once: bool = False) -> None:
    ex = make_exchange(exchange_name=str(config.get("exchange_name", "binance")), rate_limit=bool(config["rate_limit"]), market_type=str(config.get("market_type", "swap")))
    symbols = get_usdt_perp_symbols(ex)

    print(f"Exchange={config['exchange_name']} | Loaded {len(symbols)} USDT-M perpetual symbols.")
    print(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--exchange", default=DEFAULT_CONFIG["exchange_name"], choices=["binance", "mexc"])
    parser.add_argument("--market-type", default=DEFAULT_CONFIG["market_type"])
...
def main() -> None:
    ...
    config["exchange_name"] = args.exchange
