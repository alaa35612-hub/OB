...
def _process_bull_boxes_like_pine(
    ...
    while idx < len(levels):
        ...
        if high[i] < box.bottom:
            events.append(
                ZoneEvent(symbol, "zone_broken", "bull", int(ts[i]), float(high[i]), f"Bull zone broken below {box.bottom:.6f}")
            )
            if not show_broken:
                levels.pop(idx)
                deleted = True
...
def _process_bear_boxes_like_pine(
    ...
    while idx < len(levels):
        ...
        if low[i] > box.top:
            events.append(
                ZoneEvent(symbol, "zone_broken", "bear", int(ts[i]), float(low[i]), f"Bear zone broken above {box.top:.6f}")
            )
            if not show_broken:
                levels.pop(idx)
                deleted = True
...
def analyze_symbol(symbol: str, ohlcv: List[List[float]], cfg: Dict[str, object]) -> Tuple[List[Signal], List[ZoneEvent]]:
    ...
    for i in range(start_i, n):
        ...
        if is_bull_gap:
            ...
            events.append(ZoneEvent(symbol, "zone_created", "bull", int(ts[i]), float(top), f"New bull zone [{bottom:.6f}, {top:.6f}]"))
        ...
        if is_bear_gap:
            ...
            events.append(ZoneEvent(symbol, "zone_created", "bear", int(ts[i]), float(bottom), f"New bear zone [{bottom:.6f}, {top:.6f}]"))
...
def run_scan(config: Dict[str, object], once: bool = False) -> None:
    ...
    while True:
        ...
        max_symbols = int(config.get("max_symbols_per_scan", 0))
