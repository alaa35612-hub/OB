...
def analyze_symbol(symbol: str, ohlcv: List[List[float]], cfg: Dict[str, object]) -> List[Signal]:
    ...
    for i in range(start_i, n):
        ...
        boxes_bull = [b for b in boxes_bull if not ((high[i] < b.bottom) and (not bool(cfg["show_broken"]))) ]
        boxes_bear = [b for b in boxes_bear if not ((low[i] > b.top) and (not bool(cfg["show_broken"]))) ]

        if bool(cfg["show_signal"]) and i >= 1:
            for b in boxes_bull:
                if (low[i] > b.top) and (low[i - 1] <= b.top) and (not is_bull_gap):
                    signals.append(
                        Signal(
                            symbol=symbol,
                            kind="bull_break",
                            ts=int(ts[i - 1]),
                            price=float(low[i - 1]),
                            info=f"Break above Bull OB top={b.top:.6f} (gap={b.strength_pct:.3f}%)",
                        )
                    )

            for b in boxes_bear:
                if (high[i] < b.bottom) and (high[i - 1] >= b.bottom) and (not is_bear_gap):
                    signals.append(
                        Signal(
                            symbol=symbol,
                            kind="bear_break",
                            ...
                            info=f"Break below Bear OB bottom={b.bottom:.6f} (gap={b.strength_pct:.3f}%)",
                        )
                    )

        prune_overlaps_inplace_like_pine(boxes_bull)
        prune_overlaps_inplace_like_pine(boxes_bear)

        box_amount = int(cfg["box_amount"])
        if len(boxes_bull) > box_amount:
            boxes_bull = boxes_bull[-box_amount:]
        if len(boxes_bear) > box_amount:
            boxes_bear = boxes_bear[-box_amount:]
